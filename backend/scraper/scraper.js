...
function scrape() {
  request(siteBaseAddress + 'Itinerari%20Frame/titolo.htm', function(err, response, result) {
    ...
    for(var i = 0; i < links.length; i++){
        ...
        let promise = request(siteBaseAddress + 'Itinerari%20Frame/' + linkToDate, function(err, response, result) {
    ...
    return $q.all(promises).done(function() {
      ...
      let promiseToponimiPrincipali = request(siteBaseAddress + 'toponimi2.htm', function(err, response, result) {
        ...
        for(var i=0; i < righe.length; i++){
          ...
          itinerariCollegati = itinerariCollegati.split(' ').map(function(value) {
